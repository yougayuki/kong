...
local function check_schema(value)
  for _, entry in ipairs(value) do
    ...
    if (entry.stat_type == "counter" or
            entry.stat_type == "gauge") and
            ((entry.sample_rate == nil) or (entry.sample_rate ~= nil and
            type(entry.sample_rate) ~= "number") or
            (entry.sample_rate ~= nil and entry.sample_rate < 1)) then
      return false, "sample rate must be defined for counters and gauges."
    end
    if (entry.name == "status_count_per_user" or
            entry.name == "request_per_user" or
            entry.name == "unique_users") and
            entry.consumer_identifier == nil then
    ...
    if (entry.name == "status_count_per_user" or
            ...
            entry.name == "unique_users") and
            entry.consumer_identifier ~= nil then
      local identifier_ok = consumer_identifiers[entry.consumer_identifier]
    ...
    if (entry.name == "status_count" or
            entry.name == "status_count_per_user" or
            entry.name == "request_per_user") and
            entry.stat_type ~= "counter" then
